...
window.addEventListener('DOMContentLoaded', function () {
  ...
  // Urban Sectoral Functionality button event listener
  const sectoralFunctionalityBtn = document.getElementById('sectoral-functionality-btn');
  if (sectoralFunctionalityBtn) {
    sectoralFunctionalityBtn.addEventListener('click', function() {
  ...
  const applySectoralMappingBtn = document.getElementById('apply-sectoral-mapping-btn');
  ...
  console.log('Layer visibility will be controlled through the layer management interface only');

  // Symbol Editor Event Listeners
  const layerStyleSelect = document.getElementById('layerStyleSelect');
  const neighborhoodColoringSection = document.getElementById('neighborhoodColoringSection');
  const colorByColumnSelect = document.getElementById('colorByColumnSelect');
  const applyColumnColoringBtn = document.getElementById('applyColumnColoringBtn');
  ...
  // Apply column-based coloring
  if (applyColumnColoringBtn) {
    applyColumnColoringBtn.addEventListener('click', function() {
      const selectedColumn = colorByColumnSelect ? colorByColumnSelect.value : '';
      if (selectedColumn) {
        applyColumnColoring(selectedColumn);
      } else {
        alert('يرجى اختيار عمود للتلوين');
      }
    });
  }
...
function createNeighborhoodPopup(feature, layer) {
  ...
  if (sector) {
    ...
    sectorStrong.appendChild(document.createTextNode(' مركبة الحي:'));
  }

  const sectorSpan = document.createElement('span');

  // حساب المتوسط الحسابي الافتراضي
  const defaultComposite = calculateDefaultComposite();

  if (sector) {
    sectorSpan.innerHTML = `${sector} <br><small style="color: #10b981; font-weight: bold;">مركبة الحي: ${defaultComposite}</small>`;
  } else {
    sectorSpan.innerHTML = `<small style="color: #10b981; font-weight: bold;">مركبة الحي: ${defaultComposite}</small>`;
  }
  ...
  // إضافة event listener للضغط على سطر القطاع
  sectorPara.style.cursor = 'pointer';
  sectorPara.style.transition = 'background-color 0.2s ease';
  sectorPara.addEventListener('mouseenter', function() {
    this.style.backgroundColor = 'rgba(48, 102, 255, 0.1)';
  });
  sectorPara.addEventListener('mouseleave', function() {
    this.style.backgroundColor = 'transparent';
  });
  sectorPara.addEventListener('click', function() {
    openCompositeCalculationPanel(properties.ID, name, sector);
  });
...
function getFixedScoreForCriteria(value, field) {
  ...
  const criteriaScores = {
    ...
    'severe_damage': {
      '0%': 10.0, '10%': 9.0, '20%': 8.0, '30%': 7.0, '40%': 6.0,
      '50%': 5.0, '60%': 4.0, '70%': 3.0, '80%': 2.0, '90%': 1.5, '100%': 1.0
    },
  ...
  if (criteriaScores[field.key] && criteriaScores[field.key][stringValue] !== undefined) {
...
function analyzeTextValue(value, field) {
  ...
  const positiveKeywords = [
    'ممتاز', 'جيد', 'عالي', 'قوي', 'نشط', 'فعال', 'مستقر', 'آمن',
    'صحي', 'نظيف', 'منظم', 'متطور', 'حديث', 'متقدم', 'ناجح'
  ];

  // كلمات متوسطة (5-7)
  const neutralKeywords = [
    'مقبول', 'عادي', 'متوسط', 'طبيعي', 'معتدل', 'مناسب', 'كافي'
  ];

  // كلمات سلبية (1-4)
  const negativeKeywords = [
    'ضعيف', 'سيء', 'متدهور', 'خطير', 'غير آمن', 'ملوث', 'فوضوي',
    'قديم', 'متهالك', 'مهجور', 'معطل', 'مكسور', 'تالف'
  ];
  ...
  if (field && field.key) {
    ...
    if (field.key.includes('needs') || field.key.includes('notes') || field.key.includes('احتياج')) {
      return 3 + (textHash % 300) / 100; // بين 3-6 (وجود احتياجات = تقييم أقل)
    }

    // حقول الأسماء والمعرفات
    if (field.key.includes('name') || field.key.includes('id') || field.key.includes('اسم')) {
...
function showFullscreenPopup() {
  ...
  const sectoralFunctionalityContainer = document.getElementById('sectoral-functionality-container');
  ...
  if (fullscreenPopup) {
    ...
    if (sectoralFunctionalityContainer) sectoralFunctionalityContainer.style.display = 'none';
    ...
    if (popupContentContainer) {
      ...
      if (popupContentContainer.innerHTML.trim() === '<!-- Content from window.html will be loaded here -->') {
        popupContentContainer.innerHTML = `
          <div style="padding: 20px; font-family: 'Cairo', sans-serif; height: 100%; overflow-y: auto;">
            <h2 style="color: #333; margin-bottom: 20px; text-align: center;">
              <i class="fas fa-calculator" style="color: #28a745;"></i> الفعالية العمرانية المركبة
            </h2>
            ...
            <div style="background: #f8f9fa; padding: 20px; border-radius: 10px; margin-bottom: 20px;">
              ...
              <div style="text-align: center; margin-top: 15px;">
                ...
                " onmouseover="this.style.transform='scale(1.05)'" onmouseout="this.style.transform='scale(1)'">
                  <i class="fas fa-calculator"></i> حساب الفعالية المركبة
                </button>
              </div>
            </div>
            ...
            <div id="compositeResultsSection" style="display: none;">
              <h4 style="color: #495057; margin-bottom: 15px;">
                <i class="fas fa-chart-bar"></i> نتائج الفعالية العمرانية المركبة
              </h4>
...
window.showSectoralFunctionalityCalculation = showSectoralFunctionalityCalculation;
...
window.generateRandomSectoralFunctionalityForAll = generateRandomSectoralFunctionalityForAll;
...
function hideSectoralFunctionalityCalculation() {
    ...
    if (sectoralFunctionalityContainer) {
        ...
        const contentDiv = sectoralFunctionalityContainer.querySelector('div[style*="overflow-y: auto"]');
        if(contentDiv) {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if(sectoralFunctionalityContainer) {
        ...
        const closeButton = sectoralFunctionalityContainer.querySelector('.close-btn'); // Adjust selector as needed
        if(closeButton) {
            closeButton.addEventListener('click', hideSectoralFunctionalityCalculation);
        }
...
function populateSectoralFunctionalityTable() {
  const tableBody = document.querySelector('#sectoral-functionality-table tbody');
  ...
  // Sectoral functionality column names (13 columns as specified)
  const sectoralColumns = [
    'التدخلات الإنسانية',
    'الأسواق الأساسية',
    'إدارة النفايات الصلبة',
    'الحدائق والمساحات المعيشية',
    'المرافق التعليمية',
    'المراكز الصحية',
    'شبكة الكهرباء',
    'شبكة الاتصالات',
  ...
  if (typeof neighborhoodsData !== 'undefined' && neighborhoodsData.features) {
    neighborhoodsData.features.forEach((feature, index) => {
      const properties = feature.properties;
      const neighborhoodName = properties.Names || properties.Name_En || properties.Name || `الحي ${index + 1}`;
      ...
      sectoralColumns.forEach(columnName => {
        ...
        if (window.sectoralFunctionalityData[neighborhoodName] &&
          ...
          functionality = window.sectoralFunctionalityData[neighborhoodName][columnName];
        } else {
          ...
          window.sectoralFunctionalityData[neighborhoodName][columnName] = functionality;
        }

        const functionalityClass = getFunctionalityClass(functionality.status);
        const functionalityColor = getFunctionalityColor(functionality.percentage);
    ...
    console.log(`Populated sectoral functionality table with ${neighborhoodsData.features.length} neighborhoods`);
  } else {
    // Create sample data if no neighborhoods data available
    const sampleNeighborhoods = [
      'الخالدية', 'صلاح الدين', 'الميدان', 'العزيزية', 'الشعار',
      'باب النيرب', 'المحافظة', 'الجميلية', 'السبيل', 'الكلاسة'
    ];
...
function calculateSectoralFunctionality(properties, sectorName) {
  try {
    ...
    if (!tablesData) {
      ...
      return { percentage: Math.floor(Math.random() * 100), status: 'غير متاح' };
    }
  ...
  const sectorMapping = {
    ...
    'الحدائق والمساحات المعيشية': 'الحدائق_والمساحات_المعيشية',
    'المرافق التعليمية': 'المرافق_التعليمية',
    'المراكز الصحية': 'المراكز_الصحية',
  ...
  if (!tableKey || !tablesData[tableKey]) {
    return { percentage: Math.floor(Math.random() * 100), status: 'غير محدد' };
  }
  ...
  switch (sectorName) {
    case 'التدخلات الإنسانية':
      ...
      percentage = Math.round((healthScore + foodScore + clothesScore + housingScore + eduScore + learningScore + incomeScore) / 7);
      break;
    ...
    case 'إدارة النفايات الصلبة':
      ...
      const cleanlinessScores = {
        'جيدة': 100, 'مقبول': 70, 'ضعيف': 40, 'معدوم': 0
      };
      const pestControlScores = {
        'دائما': 100, 'احيانا': 60, 'لا يوجد': 0
      };
      const rubbleRemovalScores = {
        'بشكل دائم': 100, 'بشكل جزئي': 60, 'لا يوجد ترحيل': 0
      };
      const dumpingSitesScore = sampleData.dumping_sites === 'لا يوجد' ? 100 : 0;
      ...
      percentage = Math.round((dumpingSitesScore * 0.3) + (cleanScore * 0.3) + (pestScore * 0.2) + (rubbleScore * 0.2));
      break;

    case 'الحدائق والمساحات المعيشية':
      // Calculate based on park infrastructure status
      const damageScores = {
        'غير متضرر': 100, 'متضرر جزئيا': 60, 'متضرر كليا': 0
      };
      const parkStatusScores = {
        'يعمل بشكل اعتيادي': 100,
        'يعمل بشكل مقبول': 75,
        'يعمل بشكل متقطع': 40,
        'لا يعمل (معدوم)': 0
      ...
      const waterScore = damageScores[sampleData.water] || 0;
      const lightingScore = damageScores[sampleData.lighting] || 0;
      const furnitureScore = damageScores[sampleData.furniture] || 0;
      const parkStatus = parkStatusScores[sampleData.status] || 0;

      percentage = Math.round((waterScore * 0.3) + (lightingScore * 0.2) + (furnitureScore * 0.2) + (parkStatus * 0.3));
      break;

    case 'المرافق التعليمية':
      // Calculate based on educational facility status
      const infraScores = {
        'غير متضرر': 100, 'متضرر جزئيا': 60, 'متضرر كليا': 0
      };
      const availabilityScores = {
        'متاح': 100, 'متاح بحدود': 60, 'غير متاح': 0
      };
      const facilityStatusScores = {
        'يعمل بشكل اعتيادي': 100,
        'يعمل بشكل مقبول': 75,
        'يعمل بشكل متقطع': 40,
        'لا يعمل (معدوم)': 0
      ...
      const infraScore = infraScores[sampleData.infrastructure] || 0;
      const staffScore = availabilityScores[sampleData.staff] || 0;
      const suppliesScore = availabilityScores[sampleData.supplies] || 0;
      const facilityStatus = facilityStatusScores[sampleData.status] || 0;

      percentage = Math.round((infraScore * 0.3) + (staffScore * 0.25) + (suppliesScore * 0.25) + (facilityStatus * 0.2));
      break;

    case 'المراكز الصحية':
      // Same calculation as educational facilities
      const healthInfraScores = {
        'غير متضرر': 100, 'متضرر جزئيا': 60, 'متضرر كليا': 0
      };
      const healthAvailabilityScores = {
        'متاح': 100, 'متاح بحدود': 60, 'غير متاح': 0
      };
      const healthFacilityStatusScores = {
        'يعمل بشكل اعتيادي': 100,
        'يعمل بشكل مقبول': 75,
        'يعمل بشكل متقطع': 40,
        'لا يعمل (معدوم)': 0
      ...
      const healthInfraScore = healthInfraScores[sampleData.infrastructure] || 0;
      const healthStaffScore = healthAvailabilityScores[sampleData.staff] || 0;
      const healthSuppliesScore = healthAvailabilityScores[sampleData.supplies] || 0;
      const healthStatus = healthFacilityStatusScores[sampleData.status] || 0;

      percentage = Math.round((healthInfraScore * 0.3) + (healthStaffScore * 0.25) + (healthSuppliesScore * 0.25) + (healthStatus * 0.2));
      break;

    case 'شبكة الكهرباء':
      // Calculate based on electrical network damage and status
      const damageScores2 = {
        'معدوم': 100, 'خفيف': 80, 'متوسط': 50, 'شديد': 20
      };
      const networkStatusScores = {
        'تعمل': 100, 'تعمل جزئيا': 60, 'لا تعمل': 0
      };

      const transformerScore = damageScores2[sampleData.transformer_damage] || 0;
      const lineScore = damageScores2[sampleData.line_damage] || 0;
      const networkStatus = networkStatusScores[sampleData.status] || 0;

      percentage = Math.round((transformerScore * 0.4) + (lineScore * 0.3) + (networkStatus * 0.3));
      break;
    ...
    case 'شبكة الاتصالات':
      ...
      const teleTransformerScore = teleDamageScores[sampleData.transformer_damage] || 0;
      const teleLineScore = teleDamageScores[sampleData.line_damage] || 0;
      const teleNetworkStatus = teleNetworkStatusScores[sampleData.status] || 0;

      percentage = Math.round((teleTransformerScore * 0.4) + (teleLineScore * 0.3) + (teleNetworkStatus * 0.3));
      break;
    ...
    case 'إمدادات المياه':
      ...
      const waterDamageScores = {
        'منخفض': 100, 'متوسط': 70, 'مرتفع': 40, 'عالٍ جداً': 10
      };
      const waterStatusScores = {
        'يعمل كاملاً': 100, 'يعمل جزئياً': 60, 'متوقف': 0
      };
      ...
      const operationRatio = parseInt(sampleData.secondary_status?.replace('%', '')) || 0;

      percentage = Math.round((connectionScore * 0.2) + (waterDamageScore * 0.3) + (waterStatus * 0.3) + (operationRatio * 0.2));
      break;
    ...
    case 'شبكة الصرف الصحي':
      ...
      const sewerOperationRatio = parseInt(sampleData.secondary_status?.replace('%', '')) || 0;

      percentage = Math.round((sewerConnectionScore * 0.2) + (sewerDamageScore * 0.3) + (sewerStatus * 0.3) + (sewerOperationRatio * 0.2));
      break;

    case 'أضرار الإسكان':
      // Calculate based on housing damage (inverse - less damage = higher functionality)
      const severeDamage = parseInt(sampleData.severe_damage?.replace('%', '')) || 0;
      const mediumDamage = parseInt(sampleData.medium_damage?.replace('%', '')) || 0;
      const lightDamage = parseInt(sampleData.light_damage?.replace('%', '')) || 0;
      const undamagedUnits = parseInt(sampleData.undamaged_units?.replace('%', '')) || 0;

      // Higher undamaged percentage = higher functionality
      // Lower damage percentages = higher functionality
      percentage = Math.round(undamagedUnits + ((100 - severeDamage - mediumDamage - lightDamage) * 0.3));
    ...
    case 'النسيج الحضري':
      ...
      const urbanStatusScores = {
        'منظم': 100, 'مختلط': 70, 'بلدة قديمة': 60, 'عشوائي': 30
      };
      const urbanScore = urbanStatusScores[sampleData.urbn_status] || 0;
      const informalPercent = parseInt(sampleData.informal_percent?.replace('%', '')) || 0;
      const traditionalPercent = parseInt(sampleData.traditional_percent?.replace('%', '')) || 0;

      // Lower informal housing = higher functionality
      // Higher traditional housing = moderate functionality
      percentage = Math.round((urbanScore * 0.5) + ((100 - informalPercent) * 0.3) + (traditionalPercent * 0.2));
      break;

    case 'التغيرات السكانية':
      // Calculate based on population stability
      const population = parseInt(sampleData.population) || 0;
      const migrantsPercent = parseInt(sampleData.migrants?.replace('%', '')) || 0;
      const returneesPercent = parseInt(sampleData.returnees?.replace('%', '')) || 0;

      // Higher returnees = positive, lower migrants = more stable
      const stabilityScore = Math.max(0, 100 - migrantsPercent + (returneesPercent * 0.5));
  ...
  return {
    ...
    console.error('Error calculating sectoral functionality for', sectorName, ':', error);
    return {
      percentage: Math.floor(Math.random() * 100),
...
function randomizeSectoralFunctionality() {
  ...
  let neighborhoods = [];
  if (typeof neighborhoodsData !== 'undefined' && neighborhoodsData.features) {
    neighborhoods = neighborhoodsData.features.map((feature, index) => {
      const properties = feature.properties;
      return properties.Names || properties.Name_En || properties.name || properties.NAME || `الحي ${index + 1}`;
  ...
  console.log('Sectoral functionality data randomized successfully for', neighborhoods.length, 'neighborhoods');
}

// Debug function to check sectoral data
function debugSectoralData() {
  console.log('=== SECTORAL DATA DEBUG ===');
  console.log('neighborhoodsData available:', typeof neighborhoodsData !== 'undefined');
  console.log('sectoralFunctionalityData available:', !!window.sectoralFunctionalityData);

  if (typeof neighborhoodsData !== 'undefined' && neighborhoodsData.features) {
    console.log('Number of neighborhoods in GeoJSON:', neighborhoodsData.features.length);
    console.log('Sample neighborhood properties:', neighborhoodsData.features[0].properties);

    const sampleNames = neighborhoodsData.features.slice(0, 5).map(feature => {
      return {
  ...
  if (window.sectoralFunctionalityData) {
    console.log('Number of neighborhoods in sectoral data:', Object.keys(window.sectoralFunctionalityData).length);
    console.log('Sample neighborhood names in data:', Object.keys(window.sectoralFunctionalityData).slice(0, 5));

    const firstNeighborhood = Object.keys(window.sectoralFunctionalityData)[0];
    if (firstNeighborhood) {
      console.log('Sample data for', firstNeighborhood, ':', window.sectoralFunctionalityData[firstNeighborhood]);
    }
  }
...
function testSectoralColoring() {
  ...
  if (!window.sectoralFunctionalityData || Object.keys(window.sectoralFunctionalityData).length === 0) {
...
function createSimpleSectoralInterface() {
  ...
  // Sectoral functionality column names (13 columns as specified)
  const sectoralColumns = [
    { name: 'التدخلات الإنسانية', icon: 'fas fa-hands-helping', color: '#e74c3c' },
    { name: 'الأسواق الأساسية', icon: 'fas fa-store', color: '#3498db' },
    { name: 'إدارة النفايات الصلبة', icon: 'fas fa-trash', color: '#2ecc71' },
    { name: 'الحدائق والمساحات المعيشية', icon: 'fas fa-tree', color: '#27ae60' },
    { name: 'المرافق التعليمية', icon: 'fas fa-school', color: '#f39c12' },
    { name: 'المراكز الصحية', icon: 'fas fa-hospital', color: '#e67e22' },
  ...
  console.log('Simple sectoral interface created with', sectoralColumns.length, 'sectors');
}
...
function createSectoralMapsGrid() {
      ...
      ${sectoralColumns.map(sector => `
        ...
        ">
          <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 15px;">
            ...
            <h4 style="margin: 0; color: #333; font-size: 14px; font-weight: bold;">${sector.name}</h4>
          </div>
  ...
  setTimeout(() => {
    sectoralColumns.forEach(sector => {
      createMiniMapForSector(sector.name, `miniMap-${sector.name.replace(/\s+/g, '-')}`);
    });
  }, 100);
}
...
function generateRandomSectoralFunctionalityForAll() {
  ...
  neighborhoodsData.features.forEach(feature => {
    const properties = feature.properties;
    const neighborhoodName = properties.Names || properties.Name_En || properties.name || properties.NAME;
    ...
    if (neighborhoodName) {
      ...
      sectoralColumns.forEach(sectorName => {
        ...
        window.sectoralFunctionalityData[neighborhoodName][sectorName] = sectorData;
      });
    }
  });

  console.log('Generated sectoral functionality data for neighborhoods:', Object.keys(window.sectoralFunctionalityData));

  console.log('Generated sectoral functionality data for', Object.keys(window.sectoralFunctionalityData).length, 'neighborhoods');
}
...
function initializeWeightsInterface() {
  ...
  if (!window.sectorWeights) {
    ...
    sectoralColumns.forEach((sector, index) => {
      window.sectorWeights[sector.name] = index === 0 ? defaultWeight + remainder : defaultWeight;
    });
  }

  // Create weight input cards
  weightsGrid.innerHTML = sectoralColumns.map(sector => `
    <div style="
      background: white;
      border-radius: 8px;
      padding: 15px;
    ...
    ">
      <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 10px;">
        ...
        <span style="font-size: 13px; font-weight: bold; color: #333; flex: 1;">${sector.name}</span>
      </div>
  ...
  if (applyColoringBtn) {
    applyColoringBtn.addEventListener('click', function() {
      ...
      if (sectoralMappingContainer) {
        ...
        const containers = ['popup-content-container', 'sectoral-functionality-container', 'composite-functionality-container'];
...
function updateTotalWeight() {
  ...
  const total = Object.values(window.sectorWeights).reduce((sum, weight) => sum + weight, 0);
...
function calculateCompositeEfficiency() {
  ...
  const totalWeight = Object.values(window.sectorWeights).reduce((sum, weight) => sum + weight, 0);
  if (totalWeight !== 100) {
  ...
  Object.keys(window.sectoralFunctionalityData).forEach(neighborhoodName => {
    ...
    Object.keys(window.sectorWeights).forEach(sectorName => {
      const sectorData = window.sectoralFunctionalityData[neighborhoodName][sectorName];
...
function populateCompositeResultsTable() {
  ...
  const sortedNeighborhoods = Object.keys(window.compositeEfficiencyData).sort((a, b) => {
    return window.compositeEfficiencyData[b].value - window.compositeEfficiencyData[a].value;
  });
  ...
  sortedNeighborhoods.forEach(neighborhoodName => {
    ...
    row.innerHTML = `
      ...
      <td style="padding: 8px; border: 1px solid #dee2e6; text-align: center; font-weight: bold; color: ${data.color};">${data.value.toFixed(1)}%</td>
  ...
  console.log('Populated composite results table with', sortedNeighborhoods.length, 'neighborhoods');
}
...
function debugNeighborhoodDataMatching() {
  ...
  if (typeof neighborhoodsData !== 'undefined' && neighborhoodsData.features) {
    mapNeighborhoods = neighborhoodsData.features.map(feature => {
  ...
  setTimeout(() => {
    if (window.sectoralFunctionalityData && Object.keys(window.sectoralFunctionalityData).length > 0) {
      ...
      if (grid) {
        ...
        showNotification('تم إنشاء واجهة التلوين المركب. يمكنك الآن تطبيق التلوين على الخريطة.', 'info');
      }
    } else {
      // Show the regular sectoral mapping grid as fallback
      populateSectoralMappingGrid();
      showNotification('يرجى استخدام الأزرار أدناه لتطبيق التلوين القطاعي أولاً', 'info');
    }
  }, 1000);
}
...
function loadSimpleMiniMap() {
  ...
  if (!window.compositeEfficiencyData || Object.keys(window.compositeEfficiencyData).length === 0) {
  ...
  try {
    ...
    const miniNeighborhoodsLayer = L.geoJSON(neighborhoodsData, {
      style: function(feature) {
        const props = feature.properties;
        const neighborhoodName = props.Names || props.Name_En || props.name || props.NAME;
        ...
        if (window.compositeEfficiencyData && neighborhoodName) {
          ...
          const matchingDataName = findMatchingNeighborhoodName(neighborhoodName, dataNames);

          if (matchingDataName && window.compositeEfficiencyData[matchingDataName]) {
            const compositeValue = window.compositeEfficiencyData[matchingDataName].value;
      ...
      onEachFeature: function(feature, layer) {
        ...
        if (neighborhoodName && window.compositeEfficiencyData) {
          ...
          if (matchingDataName && window.compositeEfficiencyData[matchingDataName]) {
            const compositeData = window.compositeEfficiencyData[matchingDataName];
            ...
            const popupContent = `
              <div style="text-align: center; padding: 10px; min-width: 200px;">
                ...
                <div style="margin: 10px 0;">
                  <div style="font-size: 24px; font-weight: bold; margin: 10px 0; color: ${getRedColor(compositeData.value)};">
...
function createSectoralOptionsForComposite(parentGrid) {
  ...
  // Sectoral functionality column names (13 columns as specified)
  const sectoralColumns = [
    { name: 'التدخلات الإنسانية', icon: 'fas fa-hands-helping', description: 'تقييم التدخلات الإنسانية والمساعدات' },
    { name: 'الأسواق الأساسية', icon: 'fas fa-store', description: 'حالة الأسواق والمحلات التجارية' },
    { name: 'إدارة النفايات الصلبة', icon: 'fas fa-trash', description: 'نظافة الحي وإدارة النفايات' },
    { name: 'الحدائق والمساحات المعيشية', icon: 'fas fa-tree', description: 'المساحات الخضراء والترفيهية' },
    { name: 'المرافق التعليمية', icon: 'fas fa-school', description: 'المدارس والمؤسسات التعليمية' },
    { name: 'المراكز الصحية', icon: 'fas fa-hospital', description: 'المراكز الصحية والعيادات' },
    { name: 'شبكة الكهرباء', icon: 'fas fa-bolt', description: 'شبكة الكهرباء والتغذية' },
    { name: 'شبكة الاتصالات', icon: 'fas fa-wifi', description: 'شبكات الاتصالات والإنترنت' },
    { name: 'إمدادات المياه', icon: 'fas fa-tint', description: 'شبكة المياه والتوزيع' },
    { name: 'شبكة الصرف الصحي', icon: 'fas fa-water', description: 'شبكة الصرف الصحي' },
    { name: 'أضرار الإسكان', icon: 'fas fa-home', description: 'حالة المباني والإسكان' },
    { name: 'النسيج الحضري', icon: 'fas fa-city', description: 'التخطيط العمراني والنسيج' },
    { name: 'التغيرات السكانية', icon: 'fas fa-users', description: 'التغيرات في عدد السكان' }
  ];
        ...
        if (matchingDataName && window.compositeEfficiencyData[matchingDataName]) {
          ...
          const popupContent = `
            <div style="text-align: center; padding: 8px;">
              ...
              <div style="padding: 4px 8px; background: ${getRedGradientColor(compositeData.value)}; border-radius: 10px; color: white; font-size: 11px;">
  ...
  if (!window.compositeEfficiencyData || Object.keys(window.compositeEfficiencyData).length === 0) {
    ...
    setTimeout(() => {
      if (window.compositeEfficiencyData && Object.keys(window.compositeEfficiencyData).length > 0) {
        ...
        showNotification('لم يتم العثور على بيانات كافية. يرجى المحاولة مرة أخرى.', 'warning');
      }
    }, 500);
  } else {
...
function applyCompositeColoring() {
  ...
  if (!window.compositeEfficiencyData || Object.keys(window.compositeEfficiencyData).length === 0) {
    console.log('No composite data found, trying to calculate from sectoral data...');
    ...
    if (window.sectoralFunctionalityData && Object.keys(window.sectoralFunctionalityData).length > 0) {
      ...
      if (!window.compositeEfficiencyData || Object.keys(window.compositeEfficiencyData).length === 0) {
        showNotification('فشل في حساب الفعالية المركبة من البيانات القطاعية', 'error');
        return;
      }
    } else {
      // Instead of showing error, use the sectoral mapping grid interface
      console.log('No sectoral data found, showing sectoral mapping interface...');
      showCompositeColoringWithSectoralGrid();
      return;
    }
  }
  ...
  if (!neighborhoodsLayer) {
    console.log('Neighborhoods layer not found, trying alternative approach...');

    // Check if we have sectoral functionality data with neighborhood names
    if (window.sectoralFunctionalityData && Object.keys(window.sectoralFunctionalityData).length > 0) {
      console.log('Using sectoral functionality data to apply composite coloring...');
      applyCompositeColoringFromData();
      return;
    }
    ...
    setTimeout(() => {
      ...
      if (neighborhoodsLayer) {
        ...
        showNotification('لم يتم العثور على طبقة الأحياء. تأكد من تحميل الخريطة بالكامل.', 'warning');
      }
    }, 1000);
    return;
  }
...
function calculateCompositeEfficiencyFromSectoralData() {
  ...
  // Default weights (equal weights for all sectors)
  const defaultWeights = {
    'التدخلات الإنسانية': 7.69,
    'الأسواق الأساسية': 7.69,
    'إدارة النفايات الصلبة': 7.69,
    'الحدائق والمساحات المعيشية': 7.69,
    'المرافق التعليمية': 7.69,
    'المراكز الصحية': 7.69,
    'شبكة الكهرباء': 7.69,
    'شبكة الاتصالات': 7.69,
    'إمدادات المياه': 7.69,
    'شبكة الصرف الصحي': 7.69,
    'أضرار الإسكان': 7.69,
    'النسيج الحضري': 7.69,
    'التغيرات السكانية': 7.69
  };
  ...
  Object.keys(window.sectoralFunctionalityData).forEach(neighborhoodName => {
    ...
    const compositeValue = totalWeight > 0 ? totalWeightedValue / totalWeight : 0;
  ...
  console.log('Composite efficiency calculated for', Object.keys(window.compositeEfficiencyData).length, 'neighborhoods');
}
...
function findMatchingNeighborhoodName(mapName, dataNames) {
  ...
  for (const dataName of dataNames) {
    ...
    if (normalizedMapName.includes(normalizedDataName) || normalizedDataName.includes(normalizedMapName)) {
      return dataName;
    }
  }

  // Try word-based matching (split by spaces and check if any words match)
  const mapWords = normalizedMapName.split(' ').filter(word => word.length > 2);
  for (const dataName of dataNames) {
    const normalizedDataName = normalizeNeighborhoodName(dataName);
    const dataWords = normalizedDataName.split(' ').filter(word => word.length > 2);

    // Check if any significant words match
    for (const mapWord of mapWords) {
      for (const dataWord of dataWords) {
        if (mapWord === dataWord || mapWord.includes(dataWord) || dataWord.includes(mapWord)) {
          return dataName;
        }
      }
    }
...
function applyCompositeColoringFromData() {
  ...
  if (neighborhoodNames.length > 0) {
        ...
        if (layer instanceof L.GeoJSON && typeof layer.eachLayer === 'function') {
          layer.eachLayer(function(subLayer) {
            if (subLayer.feature && subLayer.feature.properties) {
              ...
              const mapNeighborhoodName = props.Names || props.Name_En || props.name || props.NAME;

              if (mapNeighborhoodName) {
                mapNeighborhoods.push(mapNeighborhoodName);

                // Try to find a matching neighborhood in our data
                const matchingDataName = findMatchingNeighborhoodName(mapNeighborhoodName, neighborhoodNames);

                if (matchingDataName && window.compositeEfficiencyData[matchingDataName]) {
                  const compositeData = window.compositeEfficiencyData[matchingDataName];
                  console.log(`Matched: "${mapNeighborhoodName}" -> "${matchingDataName}"`);
                    ...
                    const popupContent = `
                      <div style="text-align: center; padding: 10px;">
                        ...
                        <div style="display: flex; align-items: center; justify-content: center; gap: 10px; margin-bottom: 10px;">
                          <div style="width: 20px; height: 20px; background: ${compositeData.color}; border-radius: 4px; border: 1px solid #ddd;"></div>
                          <span style="font-size: 18px; font-weight: bold; color: ${compositeData.color};">${compositeData.value.toFixed(1)}%</span>
                        </div>
                        <div style="padding: 5px 10px; background: ${compositeData.color}; border-radius: 15px; color: white; font-size: 12px;">
                          ${compositeData.status}
                        </div>
    ...
    if (foundAnyLayer && processedCount > 0) {
      ...
      showNotification(`تم تطبيق تلوين الفعالية المركبة على ${processedCount} حي`, 'success');
    } else {
      ...
      showCompositeColoringWithSectoralGrid();
      showNotification('تم إنشاء واجهة التلوين البديلة. يمكنك استخدام الأزرار أدناه لتطبيق التلوين.', 'info');
    }
  } else {
...
function applyCompositeColoringToLayer(neighborhoodsLayer) {
  ...
  if (typeof neighborhoodsLayer.eachLayer !== 'function') {
    console.warn('Invalid neighborhoods layer - no eachLayer method:', neighborhoodsLayer);
  ...
  try {
    neighborhoodsLayer.eachLayer(function(layer) {
      ...
      if (neighborhoodName) {
        ...
        if (matchingDataName && window.compositeEfficiencyData[matchingDataName]) {
          const compositeData = window.compositeEfficiencyData[matchingDataName];
          console.log(`Matched: "${neighborhoodName}" -> "${matchingDataName}"`);
...
function generateRandomSectoralFunctionality(sectorName) {
  ...
  switch (sectorName) {
    ...
    case 'الحدائق والمساحات المعيشية':
      // Parks often need improvement
      percentage = Math.floor(Math.random() * 60) + 20; // 20-80%
      break;

    case 'المرافق التعليمية':
      // Educational facilities generally maintained
      percentage = Math.floor(Math.random() * 50) + 50; // 50-100%
      break;

    case 'المراكز الصحية':
      // Health centers generally maintained
      percentage = Math.floor(Math.random() * 50) + 45; // 45-95%
      break;
...
function showSectoralMappingInterface() {
  ...
  if (fullscreenPopup && sectoralMappingContainer) {
    ...
    const grid = document.getElementById('sectoralMappingGrid');
    if (grid) {
      grid.innerHTML = '<div id="sectoralMapsContainer" style="width: 100%;"></div>';
      createSectoralMapsGrid();
    }
  }
}
...
function populateSectoralMappingGrid() {
    ...
    card.innerHTML = `
      ...
      ">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
          ...
          <span id="preview-${sector.name.replace(/\s+/g, '-')}" style="font-size: 12px; color: #007bff;"></span>
        </div>
        <div style="display: flex; gap: 2px; height: 8px; border-radius: 4px; overflow: hidden;">
  ...
  console.log('Sectoral mapping grid populated with', sectoralColumns.length, 'sectors');
}
...
function populateSectoralMappingGridWithMaps() {
  ...
  console.log('Sectoral mapping grid with mini maps populated with', sectoralColumns.length, 'sectors');
}
...
function createMiniMapForSector(sectorName, containerId) {
  ...
  try {
    ...
    if (typeof neighborhoodsData !== 'undefined' && window.sectoralFunctionalityData) {
      const miniNeighborhoodsLayer = L.geoJSON(neighborhoodsData, {
        style: function(feature) {
          const neighborhoodName = feature.properties.Names || feature.properties.Name_En || feature.properties.name || feature.properties.NAME;

          if (neighborhoodName && window.sectoralFunctionalityData && window.sectoralFunctionalityData[neighborhoodName]) {
            const sectorData = window.sectoralFunctionalityData[neighborhoodName][sectorName];
        ...
        onEachFeature: function(feature, layer) {
          ...
          if (neighborhoodName && window.sectoralFunctionalityData[neighborhoodName]) {
            const sectorData = window.sectoralFunctionalityData[neighborhoodName][sectorName];
            ...
            if (sectorData && typeof sectorData.percentage !== 'undefined') {
              ...
              layer.bindTooltip(`
                <div style="text-align: center; font-size: 11px;">
                  ...
                  <span style="color: ${color};">${sectorData.percentage.toFixed(1)}%</span><br>
    ...
    container.addEventListener('click', function() {
      ...
      showNotification(`تم تطبيق تلوين ${sectorName} على الخريطة الرئيسية`, 'success');
    });
...
function applySectoralColoring(sectorName) {
  ...
  if (window.neighborhoodsLayer) {
    window.neighborhoodsLayer.eachLayer(function(layer) {
      const neighborhoodName = layer.feature.properties.Names || layer.feature.properties.Name_En || layer.feature.properties.name || layer.feature.properties.NAME;
          ...
          const popupContent = `
            <div style="text-align: center; padding: 10px;">
              ...
              <div style="display: flex; align-items: center; justify-content: center; gap: 10px; margin-bottom: 10px;">
                ...
                <span style="font-size: 16px; font-weight: bold;">${sectorData.percentage.toFixed(1)}%</span>
              </div>
              <div style="padding: 5px 10px; background: ${getStatusBackgroundColor(sectorData.status)}; border-radius: 15px; color: white; font-size: 12px;">
                ${sectorData.status}
              </div>
            </div>
          `;
...
function getStatusBackgroundColor(status) {
  switch(status) {
    case 'ممتاز': return '#0d47a1';
    case 'جيد': return '#1976d2';
    case 'متوسط': return '#42a5f5';
    case 'ضعيف': return '#90caf9';
    case 'سيء': return '#e3f2fd';
...
function calculateSectorStatistics(sectorName) {
  let total = 0;
  let excellent = 0, good = 0, average = 0, poor = 0, bad = 0;
...
function findNeighborhoodsLayer() {
  ...
  if (window.neighborhoodsLayer && typeof window.neighborhoodsLayer.eachLayer === 'function') {
  ...
  window.map.eachLayer(function(layer) {
    ...
    if (layer instanceof L.GeoJSON && typeof layer.eachLayer === 'function') {
      ...
      try {
        ...
        if (hasNeighborhoods && neighborhoodCount > 0) {
          console.log(`Found neighborhoods layer with ${neighborhoodCount} neighborhoods`);
          foundLayer = layer;
    ...
    if (layer.eachLayer && typeof layer.eachLayer === 'function' && !(layer instanceof L.GeoJSON)) {
      try {
        layer.eachLayer(function(subLayer) {
          if (foundLayer) return; // Stop if already found

          if (subLayer instanceof L.GeoJSON && typeof subLayer.eachLayer === 'function') {
            let hasNeighborhoods = false;
            let neighborhoodCount = 0;
            ...
            try {
              ...
              if (hasNeighborhoods && neighborhoodCount > 0) {
                console.log(`Found neighborhoods layer in group with ${neighborhoodCount} neighborhoods`);
                foundLayer = subLayer;
...
function populateColumnDropdown() {
  ...
  if (typeof neighborhoodsData !== 'undefined' && neighborhoodsData.features && neighborhoodsData.features.length > 0) {
    ...
    const colorableColumns = [
      { key: 'Area_Ha', label: 'المساحة (هكتار)' },
...
function applyColumnColoring(columnName) {
  ...
  if (isNumerical) {
    ...
    const ranges = [
      { min: min, max: min + (max - min) * 0.25, color: colors[0], label: `${min.toFixed(1)} - ${(min + (max - min) * 0.25).toFixed(1)}` },
      { min: min + (max - min) * 0.25, max: min + (max - min) * 0.5, color: colors[1], label: `${(min + (max - min) * 0.25).toFixed(1)} - ${(min + (max - min) * 0.5).toFixed(1)}` },
      { min: min + (max - min) * 0.5, max: min + (max - min) * 0.75, color: colors[2], label: `${(min + (max - min) * 0.5).toFixed(1)} - ${(min + (max - min) * 0.75).toFixed(1)}` },
      { min: min + (max - min) * 0.75, max: max, color: colors[3], label: `${(min + (max - min) * 0.75).toFixed(1)} - ${max.toFixed(1)}` }
    ];
    ...
    neighborhoodsLayer.eachLayer(layer => {
      ...
      const color = isNaN(value) ? '#cccccc' : getColorForValue(value, min, max);
