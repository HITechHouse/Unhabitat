...
class AdvancedLayerManager {
  ...
  setupEventListeners() {
    ...
    document
      ...
      ?.addEventListener("click", () => this.showAddLayerDialog());
    document
      .getElementById("removeLayerBtn")
      ?.addEventListener("click", () => this.removeSelectedLayer());
    document
  ...
  toggleLayerGroupVisibility(checkbox) {
    ...
    const layerItems = layerGroup.querySelectorAll(
  ...
  toggleLayerVisibility(checkbox) {
    ...
    let layer = null;
    switch (layerName) {
      case "aleppo-city-boundaries":
        layer = window.aleppoCityLayer;
        break;
    ...
    if (layer && map) {
      ...
      console.warn("Layer not found:", layerName, "Available layers:", {
        aleppoCityLayer: !!window.aleppoCityLayer,
  ...
  initializeDefaultLayers() {
    ...
    if (window.serviceSectorLabelsLayer) {
      this.layers.set(
        ...
        window.serviceSectorLabelsLayer
      );
    }
  }
  ...
  zoomToLayer(layerName) {
    ...
    if (this.layers.has(layerName)) {
      ...
      switch (layerName) {
        ...
        case "service-sectors-polygons":
          layer =
            window.serviceSectorsLayer || window.serviceSectorsGeoJsonLayer;
          break;
        case "service-sectors-labels":
          layer = window.serviceSectorLabelsLayer;
          break;
    ...
    if (layer && layer.getBounds && layer.getBounds().isValid()) {
      ...
      console.warn(
        "Cannot zoom to layer - layer not found or has no bounds:",
        layerName
      );
  ...
  setupImportHandlers() {
    ...
    // Section toggle
    document
      .getElementById("dataManagerToggle")
