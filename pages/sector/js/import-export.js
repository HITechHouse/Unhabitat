 ...
 * supports exporting all layers with all required 
...
/**
 * Simple and robust feature validation
 * @param {Object} feature - GeoJSON feature
 * @param {number} index - Feature index for logging
 * @returns {Object|null} - Feature or null if invalid
 */
function validateAndFixFeature(feature, index) {
  try {
    // Basic structure validation
    if (!feature || !feature.geometry || !feature.geometry.coordinates) {
      console.warn(`Feature ${index}: Missing basic structure`);
      return null;
    }

    const geometry = feature.geometry;
    console.log(`Feature ${index}: Type=${geometry.type}, Coords length=${Array.isArray(geometry.coordinates) ? geometry.coordinates.length : 'N/A'}`);
    
    // For most cases, if the data has the right basic structure, just return it
    // Only apply fixes for known problematic cases
    
    if (geometry.type === 'MultiPolygon') {
      const coords = geometry.coordinates;
      
      // Check if this is the malformed structure (coordinate pairs directly in array)
      // This is specific to the Aleppo city data issue
      if (Array.isArray(coords) && coords.length > 100 && 
          Array.isArray(coords[0]) && coords[0].length === 2 && 
          typeof coords[0][0] === 'number' && typeof coords[0][1] === 'number') {
        
        console.log(`Feature ${index}: Detected malformed MultiPolygon structure (${coords.length} coordinate pairs), fixing...`);
        
        // This is the malformed structure - wrap it properly
        return {
          ...feature,
          geometry: {
            ...geometry,
            coordinates: [[coords]] // Wrap in proper MultiPolygon structure
          }
        };
      }
      ...
      if (Array.isArray(coords) && coords.length > 0 && 
        ...
        console.log(`Feature ${index}: MultiPolygon appears properly formatted`);
        return feature;
      }
      
      console.warn(`Feature ${index}: Unexpected MultiPolygon structure`);
      return feature; // Return as-is and let Leaflet handle it
      
    } else if (geometry.type === 'Polygon') {
      // Basic validation for Polygon
      if (Array.isArray(geometry.coordinates) && geometry.coordinates.length > 0) {
        console.log(`Feature ${index}: Polygon validated`);
        return feature;
      }
      ...
      if (Array.isArray(geometry.coordinates) && geometry.coordinates.length >= 2) {
        console.log(`Feature ${index}: Point validated`);
        return feature;
      }
      
    } else if (geometry.type === 'LineString') {
      // Basic validation for LineString
      if (Array.isArray(geometry.coordinates) && geometry.coordinates.length >= 2) {
        console.log(`Feature ${index}: LineString validated`);
        return feature;
      }
    }

    // If we reach here, return the feature as-is if it has basic structure
    if (Array.isArray(geometry.coordinates)) {
      console.log(`Feature ${index}: Returning as-is (type: ${geometry.type})`);
      return feature;
    }

    console.warn(`Feature ${index}: Invalid structure, skipping`);
    return null;

  } catch (error) {
    console.error(`Feature ${index}: Validation error:`, error);
    
    // Last resort: return if has basic structure
    if (feature && feature.geometry && feature.geometry.coordinates) {
      console.warn(`Feature ${index}: Returning despite error`);
      return feature;
    }
...
function initImportExport() {
  ...
  // make sure aleppo city data is available as global variable
  if (typeof aleppoCityGeoJSON !== 'undefined' && !window.aleppoCityGeoJSON) {
    window.aleppoCityGeoJSON = aleppoCityGeoJSON;
    console.log("Set window.aleppoCityGeoJSON from global scope");
  }

  // Log data availability for debugging
  console.log("Data availability check:");
  console.log("- window.aleppoCityGeoJSON:", window.aleppoCityGeoJSON ? "Available" : "Not available");
  console.log("- global aleppoCityGeoJSON:", typeof aleppoCityGeoJSON !== 'undefined' ? "Available" : "Not available");
        ...
        if (file) {
          ...
          reader.onload = async function (e) {
            try {
              ...
              // Validate and fix coordinate structure for each feature
              const validatedFeatures = [];
              
              console.log("Starting validation of imported features...");
              console.log("Total features to validate:", data.features.length);
              ...
              data.features.forEach((feature, index) => {
                console.log(`Processing feature ${index + 1}:`, {
                  ...
                  coordinatesType: Array.isArray(feature.geometry?.coordinates) ? 'array' : typeof feature.geometry?.coordinates,
                  coordinatesLength: Array.isArray(feature.geometry?.coordinates) ? feature.geometry.coordinates.length : 'N/A'
                });
                ...
                if (
                  ...
                  console.warn(`تخطي العنصر رقم ${index + 1} - غير صالح في GeoJSON`);
                  return;
                }

                // Try validation first
                const correctedFeature = validateAndFixFeature(feature, index);
                if (correctedFeature) {
                  console.log(`Feature ${index + 1} validated successfully`);
                  validatedFeatures.push(correctedFeature);
                } else {
                  // If validation fails, try to use the original feature if it has basic structure
                  if (feature.geometry && Array.isArray(feature.geometry.coordinates)) {
                    console.warn(`Feature ${index + 1} failed validation but has basic structure, using as-is`);
                    validatedFeatures.push(feature);
                  } else {
                    console.warn(`Feature ${index + 1} failed validation and has no valid structure`);
                  }
                }
              });

              console.log(`Validation complete: ${validatedFeatures.length} valid features out of ${data.features.length}`);

              if (validatedFeatures.length === 0) {
                // Show detailed error information
                console.error("No valid features found. Sample feature structure:");
                if (data.features.length > 0) {
                  console.error("First feature:", JSON.stringify(data.features[0], null, 2));
                }
                throw new Error("لا توجد عناصر صالحة بعد التحقق من الإحداثيات. تحقق من بنية البيانات في الكونسول.");
              }

              const originalCount = data.features.length;
              
              // Update data with validated features
              data.features = validatedFeatures;
              
              console.log(`تم التحقق من ${validatedFeatures.length} عنصر من أصل ${originalCount} عنصر`);
...
async function exportLayer(layerName, format) {
  ...
  try {
    ...
    switch (layerName) {
      case "aleppo-city":
        data = getAleppoCityData();
        fileName = `aleppo_city_${formatDate()}`;
        break;
...
/**
 * get the aleppo city data
 */
function getAleppoCityData() {
  console.log("Attempting to get Aleppo city data...");

  let aleppoCitySource = window.aleppoCityGeoJSON || null;

  // Check if window.aleppoCityGeoJSON is not available
  if (!aleppoCitySource && typeof aleppoCityGeoJSON !== 'undefined') {
    aleppoCitySource = aleppoCityGeoJSON;
    console.log("Found aleppoCityGeoJSON in global scope");
  }

  console.log("Aleppo city source:", aleppoCitySource ? "Available" : "Not available");

  if (!aleppoCitySource) {
    console.error("No Aleppo city data source found");
    throw new Error("بيانات مدينة حلب غير متوفرة - لم يتم العثور على مصدر البيانات");
  }

  if (!aleppoCitySource.features) {
    console.error("Aleppo city data has no features property");
    console.log("Available properties:", Object.keys(aleppoCitySource));
    throw new Error("بيانات مدينة حلب غير صالحة - لا تحتوي على خاصية features");
  }

  if (!Array.isArray(aleppoCitySource.features)) {
    console.error("Features is not an array:", typeof aleppoCitySource.features);
    throw new Error("بيانات مدينة حلب غير صالحة - features ليست مصفوفة");
  }

  if (aleppoCitySource.features.length === 0) {
    console.warn("Aleppo city data has no features");
    throw new Error("بيانات مدينة حلب فارغة - لا توجد عناصر للتصدير");
  }

  console.log(`Found ${aleppoCitySource.features.length} Aleppo city features`);
  
  // Apply coordinate validation and correction to Aleppo city data
  console.log("Applying coordinate validation to Aleppo city data...");
  const validatedFeatures = [];
  
  aleppoCitySource.features.forEach((feature, index) => {
    try {
      const correctedFeature = validateAndFixFeature(feature, index);
      if (correctedFeature) {
        validatedFeatures.push(correctedFeature);
      } else {
        // If validation fails, try to use original if it has basic structure
        if (feature.geometry && Array.isArray(feature.geometry.coordinates)) {
          console.warn(`Using original feature ${index} despite validation failure`);
          validatedFeatures.push(feature);
        }
      }
    } catch (error) {
      console.error(`Error processing Aleppo city feature ${index}:`, error);
      // Try to use original feature as fallback
      if (feature.geometry && Array.isArray(feature.geometry.coordinates)) {
        console.warn(`Using original feature ${index} as fallback`);
        validatedFeatures.push(feature);
      }
    }
  });
  ...
  if (validatedFeatures.length === 0) {
    console.error("No valid features after validation, returning original data");
    return aleppoCitySource; // Return original data as last resort
  }
  
  console.log(`تم التحقق من ${validatedFeatures.length} عنصر من أصل ${aleppoCitySource.features.length} عنصر لمدينة حلب`);
...
function getNeighborhoodsData() {
  ...
  // Apply coordinate validation and correction to neighborhoods data
  console.log("Applying coordinate validation to neighborhoods data...");
  const validatedFeatures = [];
  
  neighborhoodsSource.features.forEach((feature, index) => {
    try {
      const correctedFeature = validateAndFixFeature(feature, index);
      if (correctedFeature) {
        validatedFeatures.push(correctedFeature);
        ...
        // If validation fails, try to use original if it has basic structure
        if (feature.geometry && Array.isArray(feature.geometry.coordinates)) {
          console.warn(`Using original neighborhoods feature ${index} despite validation failure`);
          validatedFeatures.push(feature);
        }
      }
    } catch (error) {
      console.error(`Error processing neighborhoods feature ${index}:`, error);
      // Try to use original feature as fallback
      if (feature.geometry && Array.isArray(feature.geometry.coordinates)) {
        console.warn(`Using original neighborhoods feature ${index} as fallback`);
        validatedFeatures.push(feature);
      }
    }
  });
  ...
  if (validatedFeatures.length === 0) {
    console.error("No valid neighborhood features after validation, returning original data");
    return neighborhoodsSource; // Return original data as last resort
  }
  
  console.log(`تم التحقق من ${validatedFeatures.length} عنصر من أصل ${neighborhoodsSource.features.length} عنصر للأحياء`);
...
function getServiceSectorsData() {
  ...
  // Apply coordinate validation and correction to service sectors data
  console.log("Applying coordinate validation to service sectors data...");
  const validatedFeatures = [];
  
  serviceSectorsSource.features.forEach((feature, index) => {
    try {
      const correctedFeature = validateAndFixFeature(feature, index);
      if (correctedFeature) {
        validatedFeatures.push(correctedFeature);
        ...
        // If validation fails, try to use original if it has basic structure
        if (feature.geometry && Array.isArray(feature.geometry.coordinates)) {
          console.warn(`Using original service sectors feature ${index} despite validation failure`);
          validatedFeatures.push(feature);
        }
      }
    } catch (error) {
      console.error(`Error processing service sectors feature ${index}:`, error);
      // Try to use original feature as fallback
      if (feature.geometry && Array.isArray(feature.geometry.coordinates)) {
        console.warn(`Using original service sectors feature ${index} as fallback`);
        validatedFeatures.push(feature);
      }
    }
  });
  ...
  if (validatedFeatures.length === 0) {
    console.error("No valid service sectors features after validation, returning original data");
    return serviceSectorsSource; // Return original data as last resort
  }
  
  console.log(`تم التحقق من ${validatedFeatures.length} عنصر من أصل ${serviceSectorsSource.features.length} عنصر لدوائر الخدمات`);
  
  // Return corrected data
  return {
    ...serviceSectorsSource,
    features: validatedFeatures
  };
}
