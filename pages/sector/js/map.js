 ...
 */
  function loadAleppoCityLayer() {
    console.log("بدء تحميل طبقة مدينة حلب...");
    
    // التحقق من وجود البيانات المحملة مسبقاً
    if (typeof aleppoCityGeoJsonData !== 'undefined') {
      console.log("استخدام البيانات المحملة مسبقاً");
      processAleppoCityData(aleppoCityGeoJsonData);
      return;
    }
    
    // محاولة تحميل البيانات من الملف (للخوادم HTTP)
    console.log("محاولة تحميل البيانات من الملف...");
    fetch('./static/aleppo.geojson')
      .then(response => {
        console.log("استجابة fetch:", response.status);
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
        }
        return response.json();
      })
      .then(processAleppoCityData)
      .catch(error => {
        console.error("خطأ في تحميل بيانات مدينة حلب من الملف:", error);
        console.log("يرجى استخدام خادم HTTP أو التأكد من وجود ملف js/aleppo-data.js");
        alert("لتجنب مشاكل CORS، يرجى تشغيل خادم HTTP أو استخدام البيانات المضمنة");
      });
  }

 /**
  * تحويل الإحداثيات من UTM Zone 37N إلى WGS84
  */
 function transformUTMCoordinates(geojsonData) {
   console.log("تحويل الإحداثيات من UTM إلى WGS84...");
   
   const transformedData = JSON.parse(JSON.stringify(geojsonData));
   
   // تعريف أنظمة الإحداثيات
   if (typeof proj4 !== 'undefined') {
     // استخدام Proj4js للتحويل الدقيق
     const utm37n = '+proj=utm +zone=37 +datum=WGS84 +units=m +no_defs';
         ...
         feature.geometry.coordinates = feature.geometry.coordinates.map(polygon => {
           return polygon.map(ring => {
             return ring.map(coord => {
               ...
               try {
                 // تحويل دقيق باستخدام Proj4js
                 const result = proj4(utm37n, wgs84, [easting, northing]);
                 return [result[0], result[1]];
               } catch (error) {
                 console.error("خطأ في تحويل الإحداثية:", error);
                 // استخدام التحويل التقريبي كبديل
                 const lon = 39.0 + ((easting - 500000) * 0.000009);
                 const lat = 36.2 + ((northing - 4000000) * 0.000009);
                 return [lon, lat];
               }
             });
           });
     ...
     // استخدام التحويل التقريبي إذا لم تكن Proj4js متوفرة
     transformedData.features.forEach(feature => {
       if (feature.geometry.type === "MultiPolygon") {
         feature.geometry.coordinates = feature.geometry.coordinates.map(polygon => {
           return polygon.map(ring => {
   ...
   console.log("تم تحويل الإحداثيات، أول إحداثية:", transformedData.features[0].geometry.coordinates[0][0][0]);
   return transformedData;
 }

/**
 * معالجة بيانات مدينة حلب وإنشاء الطبقة
 */
function processAleppoCityData(data) {
  console.log("بيانات مدينة حلب المحملة:", data);
  aleppoCityData = data;
  ...
  // تحويل الإحداثيات من UTM إلى WGS84
  let processedData = transformUTMCoordinates(data);
  
  aleppoCityLayer = L.geoJSON(processedData, {
    style: {
      fillColor: "#f59e0b",
      weight: 5,
      opacity: 1,
    ...
    onEachFeature: function(feature, layer) {
      console.log("معالجة feature:", feature.properties);
      
      // إنشاء محتوى النافذة المنبثقة لمدينة حلب
  ...
  // التأكد من أن الطبقة في المقدمة
  setTimeout(() => {
    aleppoCityLayer.bringToFront();
  }, 100);

  // تحديث المتغيرات العامة
  window.aleppoCityLayer = aleppoCityLayer;
  window.aleppoCityData = aleppoCityData;

  console.log("تم تحميل طبقة مدينة حلب بنجاح، عدد الميزات:", processedData.features.length);
  
  // التكبير على الطبقة بعد التحميل
  if (aleppoCityLayer.getBounds().isValid()) {
    console.log("حدود الطبقة:", aleppoCityLayer.getBounds());
    // لا نريد التكبير التلقائي، فقط التأكد من أن الطبقة ظاهرة
    console.log("طبقة مدينة حلب جاهزة للعرض");
  }
  ...
  const flashInterval = setInterval(() => {
    if (flashCount < 3) {
      aleppoCityLayer.setStyle({
        fillOpacity: flashCount % 2 === 0 ? 0.8 : 0.6
      });
      flashCount++;
    } else {
...
/**
 * إنشاء محتوى النافذة المنبثقة لمدينة حلب
 * @param {Object} feature - بيانات المدينة من GeoJSON
 * @param {L.Layer} layer - طبقة Leaflet للمدينة
 */
function createAleppoCityPopup(feature, layer) {
  const properties = feature.properties;
  const name = "مدينة حلب";
  const area = (properties.Shape_Area / 1000000).toFixed(2); // Convert to km²
  const perimeter = (properties.Shape_Length / 1000).toFixed(2); // Convert to km
  ...
  areaIcon.className = "fas fa-expand-arrows-alt";
  areaIcon.style.color = "#f59e0b";
  ...
  areaSpan.style.background = "rgba(245, 158, 11, 0.1)";
  ...
  perimeterIcon.className = "fas fa-ruler";
  perimeterIcon.style.color = "#f59e0b";
  ...
  perimeterSpan.style.background = "rgba(245, 158, 11, 0.1)";
  ...
  button.style.background = "linear-gradient(to right, #f59e0b, #d97706)";
...
/**
 * عرض معلومات مدينة حلب في لوحة المعلومات
 */
function showAleppoCityInfo() {
  // تعيين القطاع المختار لحلب
  if (typeof window.setSelectedSector === 'function') {
    window.setSelectedSector('aleppo-city', 'مدينة حلب');
  }
  
  // إظهار tabs-container
  ...
  // تنشيط أول تاب وعرض البيانات
  const firstTab = document.querySelector('.tab-button');
  if (firstTab && typeof window.renderInfoPanel === 'function') {
    ...
    window.renderInfoPanel(tabId, 'aleppo-city');
  }
}
...
function mapLayerToggle(layerName, visible) {
  ...
  } else if (layerName === "aleppo-city" && aleppoCityLayer) {
    if (visible) {
      if (!map.hasLayer(aleppoCityLayer)) {
        map.addLayer(aleppoCityLayer);
      }
    } else {
      if (map.hasLayer(aleppoCityLayer)) {
        map.removeLayer(aleppoCityLayer);
...
function zoomToMapLayer(layerName) {
  if (layerName === "neighborhoods" && neighborhoodsLayer) {
    ...
    zoomToAleppoCityLayer();
  }
}

/**
 * التكبير على طبقة مدينة حلب مع تأثيرات بصرية
 */
function zoomToAleppoCityLayer() {
  if (aleppoCityLayer && map.hasLayer(aleppoCityLayer)) {
    try {
      console.log("التكبير على طبقة مدينة حلب...");
      
      // التكبير على الطبقة
      map.fitBounds(aleppoCityLayer.getBounds(), {
        padding: [30, 30],
        maxZoom: 11
      });
      
      // إضافة تأثير بصري لتمييز الطبقة
      aleppoCityLayer.setStyle({
        fillOpacity: 0.8,
        weight: 6,
        color: "#dc2626"
      ...
      // العودة للنمط الطبيعي بعد ثانيتين
      setTimeout(() => {
        aleppoCityLayer.setStyle({
          fillOpacity: 0.6,
          weight: 5,
      ...
      console.error("خطأ في التكبير على طبقة مدينة حلب:", error);
      alert("خطأ في التكبير على طبقة مدينة حلب");
    }
  } else {
    console.warn("طبقة مدينة حلب غير متاحة أو غير مضافة للخريطة");
    // محاولة تحميل الطبقة إذا لم تكن موجودة
    if (typeof loadAleppoCityLayer === 'function') {
      loadAleppoCityLayer();
      setTimeout(() => {
        if (aleppoCityLayer && map.hasLayer(aleppoCityLayer)) {
          zoomToAleppoCityLayer();
      ...
      alert("طبقة مدينة حلب غير متاحة حالياً");
    }
  }
}
...
function showMapLayerInfo(layerName) {
  ...
  if (infoPanel && infoPanelTitle && infoPanelContent) {
    ...
    } else if (layerName === "aleppo-city") {
      infoPanelTitle.textContent = "معلومات طبقة مدينة حلب";

      if (aleppoCityData && aleppoCityData.features && aleppoCityData.features.length > 0) {
        const properties = aleppoCityData.features[0].properties;
        const area = (properties.Shape_Area / 1000000).toFixed(2);
        const perimeter = (properties.Shape_Length / 1000).toFixed(2);

        infoPanelContent.innerHTML = `
          <div class="futuristic-panel">
            <h4 style="margin-top: 0; color: #f59e0b;"><i class="fas fa-city"></i> مدينة حلب</h4>
            <p><strong>المساحة:</strong> ${area} كم²</p>
            <p><strong>المحيط:</strong> ${perimeter} كم</p>
            <p><strong>معرف الكائن:</strong> ${properties.OBJECTID}</p>
            <p><strong>نوع الهندسة:</strong> مضلع متعدد</p>
          </div>

          <h4 style="margin-top: 15px;">الوصف والوظائف:</h4>
          <div class="futuristic-panel">
            <p>تعرض هذه الطبقة الحدود الإدارية لمدينة حلب، ويمكن استخدامها لـ:</p>
            <ul style="padding-right: 20px; margin-top: 5px;">
              <li>تحديد الحدود الإدارية للمدينة</li>
              <li>التخطيط الحضري والإقليمي</li>
              <li>تحليل المساحات والمناطق</li>
              <li>إجراء دراسات جغرافية ومكانية</li>
            </ul>
          </div>
        `;
      } else {
        infoPanelContent.innerHTML = "<p>بيانات مدينة حلب غير متاحة</p>";
      }
    }
