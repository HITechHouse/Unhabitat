...
let aleppoCityLayer; // طبقة حدود مدينة حلب
...
function initMap() {
  ...
  // Export data variables to window for import-export functionality
  if (typeof aleppoCityGeoJSON !== 'undefined') {
    window.aleppoCityGeoJSON = aleppoCityGeoJSON;
  }
  ...
  // Re-initialize import-export functionality if available
  if (typeof initImportExport === 'function') {
    console.log('Re-initializing import-export functionality...');
    initImportExport();
  } else {
    console.warn('initImportExport function not found');
  }
...
function loadLayers() {
  ...
  // تحميل طبقة مدينة حلب
  if (typeof aleppoCityGeoJSON !== "undefined" && aleppoCityGeoJSON) {
    console.log("Loading Aleppo city layer...");
    loadAleppoCityLayer();
  } else {
    console.error("لم يتم العثور على بيانات مدينة حلب");
  }

  // التحقق من وجود بيانات الأحياء
  if (typeof neighborhoodsData !== "undefined" && neighborhoodsData && neighborhoodsData.features) {
    console.log("Loading neighborhoods layer...");
...
function loadNeighborhoodsLayer() {
  ...
  // Filter and validate features before creating the layer
  const validNeighborhoods = neighborhoodsData.features.filter(feature => {
    // Check if feature has valid geometry
    if (!feature.geometry || !feature.geometry.coordinates) {
      console.warn('Neighborhood feature without geometry:', feature.properties);
      return false;
    }
    
    // For Polygon, check coordinates
      ...
      for (let i = 0; i < coords.length; i++) {
        ...
        for (let j = 0; j < ring.length; j++) {
          ...
          if (!Array.isArray(coord) || coord.length < 2 || 
            ...
            console.warn('Invalid coordinate in neighborhood:', coord, 'in feature:', feature.properties);
            return false;
          }
        }
      }
  ...
  console.log(`Valid neighborhood features: ${validNeighborhoods.length} out of ${neighborhoodsData.features.length}`);

  if (validNeighborhoods.length === 0) {
    console.error("No valid features found in neighborhoods data");
    return;
  }

  // Create filtered GeoJSON
  const filteredNeighborhoodsData = {
    ...neighborhoodsData,
    features: validNeighborhoods
  };

  neighborhoodsLayer = L.geoJSON(filteredNeighborhoodsData, {
    style: {
      fillColor: "#3388ff",
      weight: 1,
      opacity: 1,
  ...
  filteredNeighborhoodsData.features.forEach((feature) => {
    try {
      // Validate feature geometry exists and has coordinates
      if (!feature.geometry || !feature.geometry.coordinates || 
          !Array.isArray(feature.geometry.coordinates) || 
          feature.geometry.coordinates.length === 0) {
        console.warn('Invalid geometry for feature:', feature.properties);
        return;
      }
      ...
      if (!Array.isArray(coords) || coords.length === 0) {
        console.warn('Invalid coordinates for feature:', feature.properties);
        return;
      }
      ...
      if (validCoords > 0) {
        let centerLat = sumLat / validCoords;
        let centerLng = sumLng / validCoords;
        
        // Final validation of center coordinates
        if (isNaN(centerLat) || isNaN(centerLng)) {
          console.warn('Invalid center coordinates calculated for feature:', feature.properties);
          return;
        }
        ...
        console.warn('No valid coordinates found for feature:', feature.properties);
      }
    } catch (e) {
      console.error('Error processing neighborhood label:', e, feature.properties);
    }
  });
  ...
  if (aleppoCityLayer) overlays["حدود مدينة حلب"] = aleppoCityLayer;
  if (serviceSectorsLayer) overlays["دوائر الخدمات"] = serviceSectorsLayer;
  ...
  try {
    if (neighborhoodsLayer && neighborhoodsLayer.getLayers && neighborhoodsLayer.getLayers().length > 0) {
      map.fitBounds(neighborhoodsLayer.getBounds());
    } else {
      console.warn("No neighborhoods layer available to fit bounds");
    }
  } catch (e) {
  ...
  // Ensure neighborhoods data is available globally for export
...
/**
 * تحميل طبقة حدود مدينة حلب
 */
function loadAleppoCityLayer() {
  // إزالة الطبقة الموجودة إذا كانت موجودة
  if (aleppoCityLayer) {
    map.removeLayer(aleppoCityLayer);
  }

  // Validate aleppoCityGeoJSON data before creating layer
  if (!aleppoCityGeoJSON || !aleppoCityGeoJSON.features || !Array.isArray(aleppoCityGeoJSON.features)) {
    console.error("Invalid aleppoCityGeoJSON data structure");
    return;
  }

  // Fix and validate features before creating the layer
  const validFeatures = aleppoCityGeoJSON.features.map((feature, index) => {
    // Check if feature has valid geometry
    if (!feature.geometry || !feature.geometry.coordinates) {
      console.warn('Feature without geometry:', feature);
      return null;
    }
    
    // Create a copy of the feature to modify
    const fixedFeature = JSON.parse(JSON.stringify(feature));
    
    // For MultiPolygon, check and fix structure
    if (feature.geometry.type === "MultiPolygon") {
      const coords = feature.geometry.coordinates;
      
      // Check if this is the incorrectly structured data
      if (coords.length === 1 && coords[0].length > 100 && coords[0][0] && coords[0][0].length === 2) {
        console.log('Fixing MultiPolygon structure: converting flat coordinate array to proper MultiPolygon');
        
        // coords[0] is actually the coordinate array, need to wrap it properly
        ...
        for (let i = 0; i < coordArray.length; i++) {
          ...
          if (!Array.isArray(coord) || coord.length < 2 || 
              typeof coord[0] !== 'number' || typeof coord[1] !== 'number' ||
              isNaN(coord[0]) || isNaN(coord[1])) {
            console.warn('Invalid coordinate in data:', coord, 'at position', i);
            allValid = false;
            break;
          }
        }
        ...
        if (allValid) {
          // Fix the structure: wrap the coordinate array properly for MultiPolygon
          fixedFeature.geometry.coordinates = [[coordArray]]; // MultiPolygon with one polygon with one ring
          console.log('Fixed MultiPolygon structure:', coordArray.length, 'coordinates');
          return fixedFeature;
        } else {
          return null;
        }
      } else {
        // Standard MultiPolygon validation
        let allValid = true;
        for (let i = 0; i < coords.length && allValid; i++) {
          for (let j = 0; j < coords[i].length && allValid; j++) {
            const ring = coords[i][j];
            for (let k = 0; k < ring.length && allValid; k++) {
              const coord = ring[k];
              if (!Array.isArray(coord) || coord.length < 2 || 
                  typeof coord[0] !== 'number' || typeof coord[1] !== 'number' ||
                  isNaN(coord[0]) || isNaN(coord[1])) {
                console.warn('Invalid coordinate in MultiPolygon:', coord, 'at position', i, j, k);
                allValid = false;
              }
            }
          }
        }
        return allValid ? fixedFeature : null;
      }
    }
    // For Polygon, check coordinates
      ...
      for (let i = 0; i < coords.length && allValid; i++) {
        const ring = coords[i];
        for (let j = 0; j < ring.length && allValid; j++) {
          const coord = ring[j];
          if (!Array.isArray(coord) || coord.length < 2 || 
              typeof coord[0] !== 'number' || typeof coord[1] !== 'number' ||
              isNaN(coord[0]) || isNaN(coord[1])) {
            console.warn('Invalid coordinate in Polygon:', coord, 'at position', i, j, 'in feature:', feature.properties);
            allValid = false;
          }
        }
      }
  ...
  console.log(`Valid Aleppo city features: ${validFeatures.length} out of ${aleppoCityGeoJSON.features.length}`);

  if (validFeatures.length === 0) {
    console.error("No valid features found in Aleppo city data");
    return;
  }
  ...
  aleppoCityLayer = L.geoJSON(filteredGeoJSON, {
    style: {
      fillColor: "#ff7800",
      weight: 3,
      opacity: 1,
    ...
    onEachFeature: onEachAleppoCityFeature
  });

  // إضافة الطبقة إلى المجموعة الرئيسية
  if (mainLayersGroup) {
    mainLayersGroup.addLayer(aleppoCityLayer);
  } else {
    aleppoCityLayer.addTo(map);
  }

  // تحديث المتغيرات العامة
  window.aleppoCityLayer = aleppoCityLayer;
  
  // Ensure aleppo city data is available globally for export
  if (typeof aleppoCityGeoJSON !== 'undefined' && !window.aleppoCityGeoJSON) {
    window.aleppoCityGeoJSON = aleppoCityGeoJSON;
  }

  console.log("تم تحميل طبقة مدينة حلب بنجاح");
}
...
// Function to create popup content for Aleppo city (similar to neighborhood popup)
function createAleppoCityPopup(feature, layer) {
  const properties = feature.properties;
  const name = "مدينة حلب"; // Fixed name for Aleppo city
  const area = properties.Shape_Area ? (properties.Shape_Area / 1000000).toFixed(2) : "غير متوفر"; // Convert to km²
  const perimeter = properties.Shape_Length ? (properties.Shape_Length / 1000).toFixed(2) : "غير متوفر"; // Convert to km
  ...
  // Create header with styles (using orange gradient for city)
  const header = document.createElement("h3");
  header.textContent = name;
  header.style.margin = "0";
  header.style.fontSize = "18px";
  header.style.color = "#fff";
  header.style.padding = "14px 15px";
  header.style.background = "linear-gradient(135deg, #ff7800, #ff9500)"; // Orange gradient for city
  header.style.textAlign = "center";
  header.style.fontWeight = "600";
  header.style.position = "relative";
  header.style.overflow = "hidden";
  ...
  // Create ID paragraph (using OBJECTID for city)
  const idPara = document.createElement("p");
  idPara.style.margin = "8px 0";
  idPara.style.fontSize = "14px";
  idPara.style.color = "#4a5568";
  ...
  const idIcon = document.createElement("i");
  idIcon.className = "fas fa-city";
  idIcon.style.color = "#ff7800";
  idIcon.style.fontSize = "14px";
  idIcon.style.width = "16px";
  idIcon.style.textAlign = "center";
  ...
  const idSpan = document.createElement("span");
  idSpan.textContent = properties.OBJECTID || "1";
  idSpan.style.fontWeight = "500";
  idSpan.style.background = "rgba(255, 120, 0, 0.1)";
  idSpan.style.padding = "3px 8px";
  idSpan.style.borderRadius = "4px";
  ...
  areaPara.style.justifyContent = "space-between";
  areaPara.style.padding = "8px 0";
  areaPara.style.borderBottom = "1px dashed rgba(203, 213, 224, 0.5)";
  ...
  const areaSpan = document.createElement("span");
  areaSpan.textContent = `${area} كم²`;
  areaSpan.style.fontWeight = "500";
  areaSpan.style.background = "rgba(255, 120, 0, 0.1)";
  areaSpan.style.padding = "3px 8px";
  areaSpan.style.borderRadius = "4px";
  ...
  // Create perimeter paragraph
  const perimeterPara = document.createElement("p");
  perimeterPara.style.margin = "8px 0";
  perimeterPara.style.fontSize = "14px";
  perimeterPara.style.color = "#4a5568";
  perimeterPara.style.display = "flex";
  perimeterPara.style.alignItems = "center";
  perimeterPara.style.justifyContent = "space-between";
  perimeterPara.style.padding = "8px 0";

  const perimeterStrong = document.createElement("strong");
  perimeterStrong.style.color = "#2d3748";
  perimeterStrong.style.fontWeight = "600";
  perimeterStrong.style.marginLeft = "8px";
  perimeterStrong.style.display = "flex";
  perimeterStrong.style.alignItems = "center";
  perimeterStrong.style.gap = "6px";

  const perimeterIcon = document.createElement("i");
  perimeterIcon.className = "fas fa-ruler-combined";
  perimeterIcon.style.color = "#ff7800";
  perimeterIcon.style.fontSize = "14px";
  perimeterIcon.style.width = "16px";
  perimeterIcon.style.textAlign = "center";

  perimeterStrong.appendChild(perimeterIcon);
  perimeterStrong.appendChild(document.createTextNode(" المحيط:"));

  const perimeterSpan = document.createElement("span");
  perimeterSpan.textContent = `${perimeter} كم`;
  perimeterSpan.style.fontWeight = "500";
  perimeterSpan.style.background = "rgba(255, 120, 0, 0.1)";
  perimeterSpan.style.padding = "3px 8px";
  perimeterSpan.style.borderRadius = "4px";

  perimeterPara.appendChild(perimeterStrong);
  perimeterPara.appendChild(perimeterSpan);

  // Add paragraphs to info container
  infoContainer.appendChild(idPara);
  infoContainer.appendChild(areaPara);
  infoContainer.appendChild(perimeterPara);

  // Create button for city overview
  ...
  button.onclick = function () {
    ...
    if (layer && layer.getBounds) {
      map.fitBounds(layer.getBounds(), {
        padding: [20, 20],
        maxZoom: 12, // City level zoom
      });
    }

    // Show city overview or statistics
    showCityOverview();
  };
  button.style.width = "100%";
  button.style.padding = "12px 15px";
  button.style.background = "linear-gradient(to right, #ff7800, #ff9500)"; // Orange gradient for city
  ...
  button.appendChild(document.createTextNode(" عرض إحصائيات المدينة"));
...
// Function to show city overview/statistics
function showCityOverview() {
  // Use the new city info panel from tabs.js
  if (window.setSelectedCity) {
    window.setSelectedCity("1", "مدينة حلب");
  } else {
    // Fallback to simple alert if tabs.js is not loaded
    const totalNeighborhoods = neighborhoodsData ? neighborhoodsData.features.length : 0;
    const totalServiceSectors = window.serviceSectorsData ? window.serviceSectorsData.features.length : 0;
    
    const message = `
      إحصائيات مدينة حلب:
      
      • عدد الأحياء: ${totalNeighborhoods}
      • عدد دوائر الخدمات: ${totalServiceSectors}
      • النظام الإحداثي: WGS84
      • حالة البيانات: محدثة
      
      يمكنك استكشاف الأحياء ودوائر الخدمات من خلال النقر عليها على الخريطة.
    `;
...
// Function for Aleppo city layer interaction (similar to onEachNeighborhood)
function onEachAleppoCityFeature(feature, layer) {
  // Create neighborhood-style popup for city
  createAleppoCityPopup(feature, layer);
